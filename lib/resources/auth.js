...
extractOwnUsername = (html) => {
  ...
  const username = $("div > h6 > a").text().trim();
  return username;
};
...
extractAuthCookieString = (resp) => {
  ...
  // TODO: Find a better way to extract the string, even if the order changes 
  const authCookieString = cookies[1].split(";")[0].split("=")[1];
...
extractInitCookie = (response) => {
  ...
  const initCookie = cookies[0].split(";")[0];
