...
module.exports = {
	listPrivateMessages: async (authCookieString) => {
		const authCookie = `identifier=${authCookieString}`;

		const response = await request.get(request.endpoints.PRIVATE_MESSAGES, null, null, authCookie);

		return await extractPrivateMessages(response.body);
	},
};

extractPrivateMessages = (html) => {
