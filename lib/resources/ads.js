...
module.exports = {
	...
	getAdDetails: async path => {
		const response = await request.get(path);
		return extractAdDetails(response.body);
	}
};
...
extractAds = html => {
	...
	adElements.each(function(i, elem) {
		...
		const reputation = $(this)
			.find(".uad-rating")
			.attr("title");
		const positiveRatings = parseInt(
			reputation
				.split(",")[0]
				.substr(0, reputation.split(",")[0].indexOf("p"))
				.trim()
		);
		const negativeRatings = parseInt(
			reputation
				.split(",")[1]
				.substr(0, reputation.split(",")[1].indexOf("n"))
				.trim()
		);
...
extractAdDetails = html => {
	const $ = cheerio.load(html, { decodeEntities: false });

	const title = $("div.uad-content-block > h1")
		.text()
		.trim();
	...
	const locationsString = $(timeLocationBlock)
		.find(":nth-child(3)")
		.text()
		.trim();
	...
	const descriptionAsHTML = descriptionBlock
		.children()
		.first()
		.html();
	...
	const profileElement = sellerBlock.find("span > b > a");
	...
	const positiveRatings = parseInt(
		reputationString
			.split(",")[0]
			.substr(0, reputationString.split(",")[0].indexOf("p"))
			.trim()
	);
	const negativeRatings = parseInt(
		reputationString
			.split(",")[1]
			.substr(0, reputationString.split(",")[1].indexOf("n"))
			.trim()
	);
